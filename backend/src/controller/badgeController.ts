 ...
 * @access Public
 */
export async function getBadgeDefinitions(req: Request, res: Response, next: NextFunction) {
  try {
 ...
 * @access Private
 */
export async function refreshUserBadges(req: Request, res: Response, next: NextFunction) {
  const userId = req.user?.id;
  if (!userId) {
    return next(new HttpError(401, "User not authenticated"));
  }
  ...
  try {
    ...
    const definitionsMap = new Map(allBadgeDefinitions.map(def => [def.key, def]));
    ...
    for (const key of checkedBadgeKeys) {
      ...
      const levelInfo = definition?.levels.find(l => l.level === achievedLevel);
      if (!levelInfo) {
        logger.warn(`Level info for badge ${key} at level ${achievedLevel} not found in DB. Skipping.`);
        continue;
      }
      ...
      if (existingBadge) {
        ...
        operations.push(UserBadge.create({
          user: userId,
          badgeKey: key,
    ...
    if (operations.length > 0) {
      ...
      logger.info("User badges refreshed", { userId, created: badgesCreated, updated: badgesUpdated });
    } else {
      logger.info("User badges checked, no new achievements or level-ups.", { userId });
    }
