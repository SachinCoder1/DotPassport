...
type ModalStep = 'select-wallet' | 'connecting' | 'select-account' | 'auth-account-select' | 'signing' | 'auth-success' | 'success';
...
export const NavbarWalletButton: React.FC<NavbarWalletButtonProps> = ({ onOpenModal }) => {
  const { isConnected, isAuthenticated, selectedAccount, isInitializing } = useWalletStore();

  // Auto-open modal on page load if connected but not authenticated
  useEffect(() => {
  ...
  return (
    <div className="relative">
      ...
      {/* Show warning indicator if connected but not authenticated */}
...
// Account Selection Component - Updated to show all accounts with sign option
const AccountSelector: React.FC<{
  ...
  isLoading?: boolean;
}> = ({ accounts, onSelectAccount, selectedAccount, showSignButtons = false, isLoading }) => {
  return (
    <div className="space-y-3">
      <div className="text-center mb-6">
        <h3 className="text-xl font-bold text-gray-900 mb-2">
          {showSignButtons ? 'Select Account to Authenticate' : 'Select Account'}
        </h3>
        ...
        >
          ...
          <div className="flex-1 text-left">
            <div className={`font-semibold transition-colors ${
              selectedAccount?.address === account.address ? 'text-purple-700' : 'text-gray-900'
            }`}>
  ...
  return (
    <div className="space-y-3">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600">Connect with one of our available wallet providers</p>
      </div>
...
const LoadingState: React.FC<{
  ...
  return (
    <div className="text-center py-8">
      ...
      {/* Show account being signed if provided */}
      {account && (
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 mb-6 border border-purple-200">
          <div className="flex items-center justify-center space-x-3">
            ...
            <div className="text-left">
              <div className="font-semibold text-purple-900">{account.meta.name || 'Account'}</div>
      ...
      {walletName && (
        <div className="inline-flex items-center space-x-2 bg-purple-100 text-purple-700 px-4 py-2 rounded-full">
          ...
          <span className="text-sm font-medium ml-2">
            {showSpinner ? `Awaiting signature in ${walletName}` : `Working with ${walletName}`}
          </span>
        </div>
      )}
    </div>
...
// Connected State Component - Simplified since auth is handled automatically
  ...
  return (
    <div className="text-center py-8">
      ...
      <p className="text-gray-600 mb-6">
        Welcome to DotPassport! Your identity has been verified and you're ready to start building your reputation.
      </p>
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
        <div className="flex items-center justify-center space-x-3">
          ...
          <div className="text-left">
            <div className="font-semibold text-green-900">{account.meta.name || 'Account'}</div>
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="font-semibold text-gray-900">{account.meta.name || 'Account'}</div>
...
export const WalletConnectModal: React.FC<WalletConnectModalProps> = ({ isOpen, onClose }) => {
  ...
  // Handle step transitions based on wallet state
  useEffect(() => {
    if (isConnected && accounts.length > 0) {
      if (!selectedAccount) {
        setCurrentStep('select-account');
      } else if (!isAuthenticated) {
        // User is connected with account but not authenticated
        if (isSigning) {
          ...
          if (accounts.length > 1) {
            ...
            // Single account - go directly to signing
            setCurrentStep('signing');
            // Auto-trigger login for single account
            setTimeout(() => {
              login(); // This will trigger the useEffect which handles success
            }, 100);
          }
        }
      } else if (isAuthenticated && currentStep === 'signing') {
        // Just authenticated successfully - show success screen
  ...
  }, [isConnected, accounts, selectedAccount, isAuthenticated, isLoading, isSigning, currentStep, onClose]);
  ...
  const handleAccountSelect = async (account: InjectedAccountWithMeta) => {
    ...
    // After selecting account, automatically start authentication flow
    if (accounts.length > 1) {
      setCurrentStep('auth-account-select');
    } else {
      // Single account - go directly to signing
  ...
  const handleAuthAccountSelect = async (account: InjectedAccountWithMeta) => {
    ...
    if (success) {
      // Success will be handled by useEffect which will show auth-success then auto-close
    } else {
      setCurrentStep('auth-account-select'); // Back to account selection on error
    }
  };
  ...
  const canGoBack = (currentStep === 'select-account' || currentStep === 'auth-account-select') && !isSigning;
  ...
  // Prevent closing modal during signing or auth success
  const handleClose = () => {
    if (isSigning || currentStep === 'auth-success') {
      return; // Don't allow closing during signing or success screen
    }
    onClose();
  };
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-3xl bg-white/95 backdrop-blur-xl p-6 text-left align-middle shadow-2xl transition-all border border-white/20">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-lg font-semibold text-gray-900">DotPassport</span>
                  </div>
                  <button
                    onClick={handleClose}
                  ...
                  >
                    <X className={`w-5 h-5 ${isSigning || currentStep === 'auth-success' ? 'text-gray-300' : 'text-gray-500'}`} />
                  </button>
                </div>
...
// Main component that combines everything - for backward compatibility
const PolkadotWalletConnect: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { isConnected, isAuthenticated } = useWalletStore();

  // If user is already authenticated, show minimal interface
      ...
      <>
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-200 max-w-2xl mx-auto">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-3">DotPassport Ready</h2>
            <p className="text-gray-600 mb-6">
      ...
      <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-200 max-w-2xl mx-auto">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-8">
            Use the wallet button in the navigation bar to connect and authenticate your Polkadot wallet.
          </p>
